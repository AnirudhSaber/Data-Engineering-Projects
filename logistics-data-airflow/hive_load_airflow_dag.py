...
sense_logistics_file = GCSObjectsWithPrefixExistenceSensor(
    ...
    bucket='logistic-data',
    prefix='input/logistics_',
    mode='poke',
    timeout=300,
    poke_interval=30,
    dag=dag
...
create_hive_table = DataprocSubmitHiveJobOperator(
    ...
    query="""
        ...
        STORED AS TEXTFILE
        LOCATION 'gs://logistic-data/input/'
...
archive_processed_file = BashOperator(
    ...
    bash_command=f"gsutil -m mv gs://logistic-data/input/logistics_*.csv gs://logistic-data/archive",
    dag=dag
)
